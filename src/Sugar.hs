...
desugarVar (Var nume) = makeIndexedVar nume

-- >>> desugarVar (Var "x")
-- IndexedVar {ivName = "x", ivCount = 0}

sugarVar :: IndexedVar -> Var
sugarVar (IndexedVar nume nr) = if nr == 0 then (Var nume) else (Var (nume ++ "_" ++ show nr))
...
desugarExp [a, b] = : a (: b Nil)
desugarExp (h : t) = : h (desugarExp t)
-- desugarExp (Nat x) = 
